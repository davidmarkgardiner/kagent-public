...
Workflow: k8s-event-triage
    │
    ├─ parse-otlp (python:3.11-alpine)
    │   - Walks resourceLogs[].scopeLogs[].logRecords[]
    │   - Extracts attributes + parses body.stringValue
    │   - Filters: Warning events only
    │   - Output: JSON array of normalized events
    │
    └─ classify-and-alert (withParam fan-out)
        - Per event: severity classification + Telegram alert
...
To test the workflow manually without Event Hub:

```bash
# Save the sample OTLP JSON above to a file
...
# Submit workflow with the sample payload
argo submit -n argo-events \
  --from workflowtemplate/k8s-event-triage \
  -p "otlp-payload=$(cat /tmp/sample-otlp.json)" \
  --watch
```
