...
credentials into a shared writable kubeconfig inside its pod, and it must not
share a broad Azure identity with the read-only Kubernetes MCP endpoint.

The morning credential job may use Azure management-plane APIs to discover
approved clusters and obtain kubeconfig metadata. That does not require
