#!/usr/bin/env sh
# Standalone FastMCP/UAMI bundle live verifier.
set -eu
...
kubectl --context "$context" -n fastmcp-entra-poc \
  rollout status deployment/fastmcp-postgres-entra --timeout=3m

pod="$(kubectl --context "$context" -n fastmcp-entra-poc get pod \
  -l app.kubernetes.io/name=fastmcp-postgres-entra \
  -o jsonpath='{.items[0].metadata.name}')"

kubectl --context "$context" -n fastmcp-entra-poc exec "$pod" -- \
  python /app/verify_live.py

kubectl --context "$context" -n kagent wait --for=condition=Accepted \
  remotemcpserver/fastmcp-postgres-entra --timeout=3m
kubectl --context "$context" -n kagent wait --for=condition=Ready \
  agent/fastmcp-postgres-entra --timeout=3m

kubectl --context "$context" -n kagent get remotemcpserver \
...
kubectl --context "$context" -n kagent get agent fastmcp-postgres-entra \
  -o json | jq -e '
    (.status.conditions[]? | select(.type == "Accepted" and .status == "True"))
...
"$root/scripts/kagent-a2a-invoke.sh" \
  --agent fastmcp-postgres-entra --ns kagent --context "$context" --timeout 180 \
  --receipt-file "$receipt_file" --json \
  --text 'How many namespaces are in the approved inventory? Use only the approved tools and begin the final answer with FASTMCP_ENTRA_A2A_OK.' \
  >"$result"

jq -e '.ok == true and (.text | test("FASTMCP_ENTRA_A2A_OK"))' "$result" >/dev/null
...
echo "FASTMCP_ENTRA_A2A_TOOL_CALLS_OK"
echo "FASTMCP_ENTRA_LIVE_VERIFY_PASS"
