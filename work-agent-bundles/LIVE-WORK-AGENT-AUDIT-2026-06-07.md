...
| A2A endpoint | partially ready | Agent-card and health endpoints worked, but direct `message/send` smoke timed out after 180 seconds. |
| Model backend | blocked | ModelConfig was Accepted, but the backend model pod was not Ready due to GPU device allocation failure. |
...
was not Ready. Direct A2A smoke reached the agent service and created kagent
session/task records, then timed out without a response.
...
Required work-side action: decide whether the work demo uses the official MCP
with a valid token/config, or clearly labels the lite MCP as demo-only fallback.
...
Required work-side action: policy/governance audit should fail any front-door or
general triage agent with delete, exec, apply, patch, create, label, annotation,
or broad GitLab write tools unless explicitly classed as a HITL-gated
...
| `runtime-model-gateway-readiness/` | Added by this audit | Run before all live demos. |
...
| `gitlab-mcp-gitops-pr/` | Blocked for official MCP | Lite demo fallback is available but narrower than full GitLab MCP. |
| `a2a-smart-triage-workflows/` | Blocked until runtime healthy | A2A endpoint exists, but message/send timed out. |
...
| `policy-governance-safety/` | Needs live enforcement focus | Audit actual tools, not labels/descriptions. |
...
1. Run `runtime-model-gateway-readiness/`.
2. Fix model backend readiness or select an approved fallback model.
3. Fix official GitLab MCP auth or explicitly use the lite fallback for demo only.
4. Re-run A2A smoke after model readiness is restored.
5. Run governance audit against actual agent tool lists.
6. Start SRE adoption/game-day only after the preflight returns go or go-with-known-blockers.
