# Local Chaos Examples

These examples make this lifecycle bundle self-contained for the phase-1 proof:

```text
chaos action -> event observed -> Kagent triage -> lifecycle evaluation -> GitLab evidence
```

They are copied from the chaos reliability remediation bundle and are sanitized
for work-side adaptation. They are not production-ready defaults.

## Important: No New ChaosTest CRD

`ChaosTest` is a request/config contract in this handover. It is not a
Kubernetes CRD to install for phase 1.
