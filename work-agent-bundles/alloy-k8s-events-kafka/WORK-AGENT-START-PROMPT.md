...
12. Apply `06-smoke-event-job.yaml` or create an equivalent harmless Event in
    the test namespace.
13. Confirm Alloy logs show Kubernetes event source startup and no Kafka export
    errors.
14. Confirm the Argo EventSource consumer group consumed at least one Kafka
    record from the topic.
15. Confirm the Sensor created an Argo Workflow.
16. Capture Workflow logs showing the consumed payload.
17. Decide whether schema validation remains consumer-side or can move to
    broker-side validation after the actual wire format is known.
18. Clean up the smoke Job/Event unless the environment owner asks to keep it.
19. Fill in `evidence/EVIDENCE-TEMPLATE.md`.
